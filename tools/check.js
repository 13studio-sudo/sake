...
async function renderPass() {
  ...
  w.scrollTo = function () {};
  for (let i = 0; i < 600; i++) {
    if (w.SAKE && w.SAKE.pages && Object.keys(w.SAKE.pages).length >= PAGES.length && w.SAKE.render) break;
    await sleep(25);
  }
  if (!w.SAKE || !w.SAKE.render) { errors.push("index.html: SAKE never finished loading"); return; }

  const loaded = Object.keys(w.SAKE.pages).length;
  if (loaded < PAGES.length) errors.push("index.html: only " + loaded + " of " + PAGES.length + " pages loaded");

  for (const key of PAGES) {
    const page = w.SAKE.pages[key];
    ...
    for (const l of LANGS) {
      ...
      if (d.documentElement.lang !== w.SAKE.HTMLLANG[l]) errors.push(key + " [" + l + "]: html lang not set");
...
function langWalk(v, trail, key) {
  ...
  for (const k of Object.keys(v)) {
    ...
    if (typeof val === "string") {
      ...
      if (key === "kanji") continue;            /* the page about characters quotes both scripts */
      if (k === "zh") {
